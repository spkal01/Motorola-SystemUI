    ...
    private final BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            if (!BaseWifiTracker.this.mIsStarted) {
                boolean unused = BaseWifiTracker.this.mIsStarted = true;
                BaseWifiTracker.this.handleOnStart();
            }
            String action = intent.getAction();
            if (BaseWifiTracker.isVerboseLoggingEnabled()) {
                String access$100 = BaseWifiTracker.this.mTag;
                Log.v(access$100, "Received broadcast: " + action);
            }
            if ("android.net.wifi.WIFI_STATE_CHANGED".equals(action)) {
                ...
                Set access$600 = BaseWifiTracker.this.mRequestedScoreKeys;
                Objects.requireNonNull(access$600);
                networkScoreManager.requestScores((Collection) map.filter(new BaseWifiTracker$1$$ExternalSyntheticLambda1(access$600)).collect(Collectors.toList()));
    ...
    private final ConnectivityManager.NetworkCallback mDefaultNetworkCallback = new ConnectivityManager.NetworkCallback() {
        public void onCapabilitiesChanged(Network network, NetworkCapabilities networkCapabilities) {
            ...
            if (baseWifiTracker2.mIsWifiDefaultRoute != z || baseWifiTracker2.mIsCellDefaultRoute != z2) {
                if (BaseWifiTracker.isVerboseLoggingEnabled()) {
                    String access$100 = BaseWifiTracker.this.mTag;
                    Log.v(access$100, "Wifi is the default route: " + BaseWifiTracker.this.mIsWifiDefaultRoute);
                    String access$1002 = BaseWifiTracker.this.mTag;
                    Log.v(access$1002, "Cell is the default route: " + BaseWifiTracker.this.mIsCellDefaultRoute);
                }
            ...
            if (BaseWifiTracker.this.isPrimaryWifiNetwork(networkCapabilities)) {
                ...
                if (BaseWifiTracker.isVerboseLoggingEnabled()) {
                    ...
                    if (baseWifiTracker2.mIsWifiValidated != z) {
                        String access$100 = baseWifiTracker2.mTag;
                        Log.v(access$100, "Is Wifi validated: " + BaseWifiTracker.this.mIsWifiValidated);
                    }
                }
        ...
        public void postScan() {
            if (BaseWifiTracker.this.mWifiManager.startScan()) {
                ...
                if (i >= 3) {
                    if (BaseWifiTracker.isVerboseLoggingEnabled()) {
                        String access$100 = BaseWifiTracker.this.mTag;
                        Log.v(access$100, "Scanner failed to start scan " + this.mRetry + " times!");
                    }
                    this.mRetry = 0;
                    return;
                }
