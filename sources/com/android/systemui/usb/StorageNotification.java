...
public class StorageNotification extends SystemUI {
            ...
            for (VolumeRecord volumeRecord : this.mStorageManager.getVolumeRecords()) {
                if (volumeRecord.getType() == 1) {
                    ...
                    if ((findVolumeByUuid == null || !findVolumeByUuid.isMountedWritable()) && !volumeRecord.isSnoozed()) {
                        if (findVolumeByUuid == null || findVolumeByUuid.getState() != 1) {
                            str2 = this.mContext.getString(17040196, new Object[]{volumeRecord.getNickname()});
                            str = this.mContext.getString(17040195);
                        } else {
                            str2 = this.mContext.getString(17040193, new Object[]{volumeRecord.getNickname()});
                            str = this.mContext.getString(17040192);
                        }
                        Notification.Builder extend = new Notification.Builder(this.mContext, NotificationChannels.STORAGE).setSmallIcon(17303706).setColor(this.mContext.getColor(17170460)).setContentTitle(str2).setContentText(str).setContentIntent(buildForgetPendingIntent(volumeRecord)).setStyle(new Notification.BigTextStyle().bigText(str)).setVisibility(1).setLocalOnly(true).setCategory("sys").setDeleteIntent(buildSnoozeIntent(fsUuid)).extend(new Notification.TvExtender());
                        SystemUI.overrideNotificationAppName(this.mContext, extend, false);
                        extend.build().flags |= 32;
    ...
    public void onDiskScannedInternal(DiskInfo diskInfo, int i) {
        ...
        String string = this.mContext.getString(17040226, new Object[]{diskInfo.getDescription()});
        String string2 = this.mContext.getString(17040225, new Object[]{diskInfo.getDescription()});
    ...
    private void onPublicVolumeStateChangedInternal(VolumeInfo volumeInfo) {
        ...
        if (volumeInfo.getMountUserId() == -10000) {
            Log.w("StorageNotification", "VolumeInfo's user id is USER_NULL");
    ...
    private Notification onVolumeChecking(VolumeInfo volumeInfo) {
        return buildNotificationBuilder(volumeInfo, this.mContext.getString(17040193, new Object[]{this.mStorageManager.getBestVolumeDescription(volumeInfo)}), this.mContext.getString(17040192, new Object[]{this.mStorageManager.getBestVolumeDescription(volumeInfo)})).setCategory("progress").setOngoing(true).build();
    }
    ...
    private Notification onVolumeMounted(VolumeInfo volumeInfo) {
        ...
        if (findRecordByUuid == null) {
            Log.d("StorageNotification", "rec == null");
            return null;
        } else if (findRecordByUuid.isSnoozed() && disk.isSd()) {
            Log.d("StorageNotification", "rec.isSnoozed() == true & disk.isSd() == true.");
            return null;
        } else if (!disk.isAdoptable() || findRecordByUuid.isInited()) {
            String bestVolumeDescription = this.mStorageManager.getBestVolumeDescription(volumeInfo);
            String string = this.mContext.getString(17040207, new Object[]{this.mStorageManager.getBestVolumeDescription(volumeInfo)});
            PendingIntent buildBrowsePendingIntent = buildBrowsePendingIntent(volumeInfo);
            Notification.Builder category = buildNotificationBuilder(volumeInfo, bestVolumeDescription, string).addAction(new Notification.Action(17303331, this.mContext.getString(17040191), buildBrowsePendingIntent)).addAction(new Notification.Action(17303308, this.mContext.getString(17040220), buildUnmountPendingIntent(volumeInfo))).setContentIntent(buildBrowsePendingIntent).setCategory("sys");
            if (disk.isSd()) {
                category.setDeleteIntent(buildSnoozeIntent(volumeInfo.getFsUuid()));
            }
            return category.build();
        } else {
            String bestVolumeDescription2 = this.mStorageManager.getBestVolumeDescription(volumeInfo);
            String string2 = this.mContext.getString(17040203, new Object[]{this.mStorageManager.getBestVolumeDescription(volumeInfo)});
            ...
            if (isAutomotive()) {
                return buildNotificationBuilder(volumeInfo, bestVolumeDescription2, string2).setContentIntent(buildUnmountPendingIntent).setDeleteIntent(buildSnoozeIntent(volumeInfo.getFsUuid())).build();
            }
            return buildNotificationBuilder(volumeInfo, bestVolumeDescription2, string2).addAction(new Notification.Action(17303712, this.mContext.getString(17040194), buildInitPendingIntent)).addAction(new Notification.Action(17303308, this.mContext.getString(17040220), buildUnmountPendingIntent)).setContentIntent(buildInitPendingIntent).setDeleteIntent(buildSnoozeIntent(volumeInfo.getFsUuid())).build();
        }
    }

    private Notification onVolumeEjecting(VolumeInfo volumeInfo) {
        return buildNotificationBuilder(volumeInfo, this.mContext.getString(17040224, new Object[]{this.mStorageManager.getBestVolumeDescription(volumeInfo)}), this.mContext.getString(17040223, new Object[]{this.mStorageManager.getBestVolumeDescription(volumeInfo)})).setCategory("progress").setOngoing(true).build();
    }

    private Notification onVolumeUnmountable(VolumeInfo volumeInfo) {
        PendingIntent pendingIntent;
        String string = this.mContext.getString(17040222, new Object[]{this.mStorageManager.getBestVolumeDescription(volumeInfo)});
        String string2 = this.mContext.getString(17040221, new Object[]{this.mStorageManager.getBestVolumeDescription(volumeInfo)});
        if (isAutomotive()) {
    ...
    private Notification onVolumeRemoved(VolumeInfo volumeInfo) {
        ...
        return buildNotificationBuilder(volumeInfo, this.mContext.getString(17040206, new Object[]{this.mStorageManager.getBestVolumeDescription(volumeInfo)}), this.mContext.getString(17040205, new Object[]{this.mStorageManager.getBestVolumeDescription(volumeInfo)})).setCategory("err").build();
    }
    ...
    private Notification onVolumeBadRemoval(VolumeInfo volumeInfo) {
        ...
        return buildNotificationBuilder(volumeInfo, this.mContext.getString(17040190, new Object[]{this.mStorageManager.getBestVolumeDescription(volumeInfo)}), this.mContext.getString(17040189, new Object[]{this.mStorageManager.getBestVolumeDescription(volumeInfo)})).setCategory("err").build();
    }
    ...
    public void onMoveProgress(MoveInfo moveInfo, int i, long j) {
        ...
        if (!TextUtils.isEmpty(moveInfo.label)) {
            str = this.mContext.getString(17040199, new Object[]{moveInfo.label});
        } else {
            str = this.mContext.getString(17040202);
        }
        if (j < 0) {
            charSequence = null;
        } else {
        ...
        Notification.Builder ongoing = new Notification.Builder(this.mContext, NotificationChannels.STORAGE).setSmallIcon(17303706).setColor(this.mContext.getColor(17170460)).setContentTitle(str).setContentText(charSequence).setContentIntent(pendingIntent).setStyle(new Notification.BigTextStyle().bigText(charSequence)).setVisibility(1).setLocalOnly(true).setCategory("progress").setProgress(100, i, false).setOngoing(true);
    ...
    public void onMoveFinished(MoveInfo moveInfo, int i) {
        ...
        if (i == -100) {
            str = this.mContext.getString(17040201);
            str2 = this.mContext.getString(17040200, new Object[]{bestVolumeDescription});
        } else {
            str = this.mContext.getString(17040198);
            str2 = this.mContext.getString(17040197);
        }
        ...
        if (primaryStorageCurrentVolume == null || primaryStorageCurrentVolume.getDisk() == null) {
            pendingIntent = (primaryStorageCurrentVolume == null || findVolumeByQualifiedUuid == null) ? null : buildVolumeSettingsPendingIntent(findVolumeByQualifiedUuid);
        } else {
            pendingIntent = buildWizardReadyPendingIntent(primaryStorageCurrentVolume.getDisk());
        }
        Notification.Builder autoCancel = new Notification.Builder(this.mContext, NotificationChannels.STORAGE).setSmallIcon(17303706).setColor(this.mContext.getColor(17170460)).setContentTitle(str).setContentText(str2).setContentIntent(pendingIntent).setStyle(new Notification.BigTextStyle().bigText(str2)).setVisibility(1).setLocalOnly(true).setCategory("sys").setAutoCancel(true);
    ...
    private PendingIntent buildBrowsePendingIntent(VolumeInfo volumeInfo) {
        ...
        try {
            ...
            buildBrowseIntentForUser.setFlags(524288);
    ...
    private void listenForUserUnlocked() {
        this.mContext.registerReceiverAsUser(new BroadcastReceiver() {
            public void onReceive(Context context, Intent intent) {
                ...
                for (VolumeInfo access$000 : StorageNotification.this.mStorageManager.getVolumes()) {
                    StorageNotification.this.onVolumeStateChangedInternal(access$000);
                }
                StorageNotification.this.mContext.unregisterReceiver(this);
            }
        }, UserHandle.ALL, new IntentFilter("android.intent.action.USER_UNLOCKED"), (String) null, (Handler) null);
    }
    ...
    public void onConfigurationChanged(Configuration configuration) {
        ...
        if (DEBUG) {
            ...
            sb.append("mCurrentLocale: ");
            sb.append(this.mCurrentLocale);
            sb.append("  newLocale: ");
            sb.append(locale);
            sb.append("   different: ");
            sb.append(!this.mCurrentLocale.equals(locale));
