...
public class BrightnessMirrorController implements CallbackController<BrightnessMirrorListener> {
    ...
    public BrightnessMirrorController(NotificationShadeWindowView notificationShadeWindowView, NotificationPanelViewController notificationPanelViewController, NotificationShadeDepthController notificationShadeDepthController, BrightnessSlider.Factory factory, Consumer<Boolean> consumer) {
        ...
        if (MotoFeature.getInstance(notificationShadeWindowView.getContext()).isCustomPanelView() && (frameLayout = this.mBrightnessMirror) != null) {
            frameLayout.setBackgroundResource(R$drawable.brightness_mirror_background_prc);
        }
    }
    ...
    private void reinflate() {
        ...
        this.mBrightnessMirror = (FrameLayout) LayoutInflater.from(this.mBrightnessMirror.getContext()).inflate(R$layout.brightness_mirror_container, this.mStatusBarWindow, false);
        if (MotoFeature.getInstance(this.mStatusBarWindow.getContext()).isCustomPanelView() && (frameLayout = this.mBrightnessMirror) != null) {
            frameLayout.setBackgroundResource(R$drawable.brightness_mirror_background_prc);
        }
        this.mToggleSliderController = setMirrorLayout();
