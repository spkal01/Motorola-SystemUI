...
public class RemoteInputView extends LinearLayout implements View.OnClickListener {
    ...
    public RemoteInputView(Context context, AttributeSet attributeSet) {
        ...
        TypedArray obtainStyledAttributes = getContext().getTheme().obtainStyledAttributes(new int[]{16843829, 17956910});
    ...
    public void setBackgroundTintColor(int i, boolean z) {
        ...
        if (z != this.mColorized || i != this.mTint) {
            ...
            if (z) {
                ...
                int color = this.mContext.getColor(R$color.remote_input_hint);
                i2 = colorStateList2.getDefaultColor();
                TypedArray obtainStyledAttributes = getContext().getTheme().obtainStyledAttributes(new int[]{17956912, 17956913});
                try {
    ...
    private Intent prepareRemoteInputFromData(String str, Uri uri) {
        ...
        if (!TextUtils.isEmpty(this.mEditText.getText())) {
            label = "\"" + label + "\" " + this.mEditText.getText();
        }
        NotificationEntry notificationEntry = this.mEntry;
    ...
    public void sendRemoteInput(Intent intent) {
        ...
        if (bouncerChecker == null || !bouncerChecker.showBouncerIfNecessary()) {
            ...
            for (OnSendRemoteInputListener onSendRemoteInput : this.mOnSendListeners) {
        ...
        for (OnSendRemoteInputListener onSendRequestBounced : this.mOnSendListeners) {
