...
public class NotificationInterruptStateProviderImpl implements NotificationInterruptStateProvider {
    ...
    public NotificationInterruptStateProviderImpl(ContentResolver contentResolver, PowerManager powerManager, IDreamManager iDreamManager, AmbientDisplayConfiguration ambientDisplayConfiguration, NotificationFilter notificationFilter, BatteryController batteryController, StatusBarStateController statusBarStateController, HeadsUpManager headsUpManager, Handler handler) {
        ...
        C15921 r3 = new ContentObserver(handler) {
    ...
    public boolean shouldHeadsUp(NotificationEntry notificationEntry) {
        ...
        if (!MotoFeature.getExistedInstance().isSupportCli() || this.mStatusBarStateController.getState() != 1 || !HeadsUpUtil.shouldCliHeadsUpForLock((Context) null, notificationEntry.getSbn())) {
            return shouldHeadsUpWhenDozing(notificationEntry);
        }
        if (shouldHeadsUpForCliWhenDozing(notificationEntry)) {
            notificationEntry.cliForcedHeadsUp = true;
        } else {
            notificationEntry.cliForcedHeadsUp = false;
        }
        return notificationEntry.cliForcedHeadsUp;
    }

    public boolean shouldLaunchFullScreenIntentWhenAdded(NotificationEntry notificationEntry) {
        if (MotoFeature.getExistedInstance().isSupportCli() && this.mStatusBarStateController.getState() == 1 && notificationEntry.getSbn().getNotification().fullScreenIntent != null && HeadsUpUtil.shouldCliHeadsUpForLock((Context) null, notificationEntry.getSbn())) {
            return !shouldHeadsUp(notificationEntry);
        }
    ...
    private boolean shouldHeadsUpForCliWhenDozing(NotificationEntry notificationEntry) {
        StatusBarNotification sbn = notificationEntry.getSbn();
        if (this.mBatteryController.isAodPowerSave()) {
            Log.d("InterruptionStateProvider", "CLI no pulsing: disabled by battery saver: " + sbn.getKey());
            return false;
        } else if (!canAlertCommon(notificationEntry)) {
            Log.d("InterruptionStateProvider", "CLI no pulsing: notification shouldn't alert: " + sbn.getKey());
            return false;
        } else if (notificationEntry.shouldSuppressAmbient()) {
            Log.d("InterruptionStateProvider", "CLI no pulsing: ambient effect suppressed: " + sbn.getKey());
            return false;
        } else if (notificationEntry.getImportance() >= 3) {
            return true;
        } else {
            Log.d("InterruptionStateProvider", "CLI no pulsing: not important enough: " + sbn.getKey());
            return false;
        }
    }
