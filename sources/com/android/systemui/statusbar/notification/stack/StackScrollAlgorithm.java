...
public class StackScrollAlgorithm {
    ...
    private void initAlgorithmState(StackScrollAlgorithmState stackScrollAlgorithmState, AmbientState ambientState) {
        ...
        int unused = stackScrollAlgorithmState.mCurrentYPosition = -scrollY;
        int unused2 = stackScrollAlgorithmState.mCurrentExpandedYPosition = -stackScrollAlgorithmState.scrollY;
        int childCount = this.mHostView.getChildCount();
    ...
    private void updatePositionsForState(StackScrollAlgorithmState stackScrollAlgorithmState, AmbientState ambientState) {
        ...
        this.mStackScrollHeightExtent = (float) stackScrollAlgorithmState.mCurrentYPosition;
    }
    ...
    public void updateChild(int i, StackScrollAlgorithmState stackScrollAlgorithmState, AmbientState ambientState) {
        ...
        if (!(ambientState.isShadeExpanded() && expandableView == ambientState.getTrackedHeadsUpRow())) {
            if (ambientState.isOnKeyguard()) {
                ...
                if (panelController == null || !this.mIsPrcCustom || !panelController.isOpenNotificationState()) {
                    viewState.alpha = Interpolators.getNotificationScrimAlpha(ambientState.getExpansionFraction(), true);
                } else {
                    viewState.alpha = ambientState.getExpansionFraction();
                }
            }
        }
