...
public class StackStateAnimator {
    ...
    public StackStateAnimator(ViewGroup viewGroup) {
        this.mHostLayout = viewGroup;
        this.mGoToFullShadeAppearingTranslation = viewGroup.getContext().getResources().getDimensionPixelSize(R$dimen.go_to_full_shade_appearing_translation);
        this.mPulsingAppearingTranslation = viewGroup.getContext().getResources().getDimensionPixelSize(R$dimen.pulsing_notification_appear_translation);
            ...
            if (i == 0) {
                ...
                if (i == 1) {
                    if (expandableView.getVisibility() != 0) {
                        ...
                        expandableView.performRemoveAnimation(464, 0, f, false, 0.0f, new StackStateAnimator$$ExternalSyntheticLambda1(expandableView), (AnimatorListenerAdapter) null);
                    }
                } else if (i == 2) {
                    if (isFullySwipedOut(expandableView) && expandableView.getTransientContainer() != null) {
                    ...
                    if (expandableView.getParent() == null) {
                        ...
                        stackStateAnimator$$ExternalSyntheticLambda0 = new StackStateAnimator$$ExternalSyntheticLambda0(expandableView);
                    }
                    StackStateAnimator$$ExternalSyntheticLambda0 stackStateAnimator$$ExternalSyntheticLambda02 = stackStateAnimator$$ExternalSyntheticLambda0;
                    float f2 = 0.0f;
                    if (expandableView instanceof ExpandableNotificationRow) {
                    ...
                    float f3 = f2;
                    if (z) {
                        this.mAnimationProperties.delay += expandableView.performRemoveAnimation(400, 0, 0.0f, true, f3, stackStateAnimator$$ExternalSyntheticLambda02, getGlobalAnimationFinishedListener());
                    } else if (stackStateAnimator$$ExternalSyntheticLambda02 != null) {
                        stackStateAnimator$$ExternalSyntheticLambda02.run();
                    }
                }
            }
    ...
    public void animateOverScrollToAmount(float f, final boolean z, final boolean z2) {
        float currentOverScrollAmount = getCurrentOverScrollAmount(z);
        if (f != currentOverScrollAmount) {
            ...
            ofFloat.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                public void onAnimationUpdate(ValueAnimator valueAnimator) {
                    StackStateAnimator.this.setOverScrollAmount(((Float) valueAnimator.getAnimatedValue()).floatValue(), z, false, false, z2);
                }
            });
    ...
    private ExpandableView getLastChildNotGone() {
        ViewGroup viewGroup = this.mHostLayout;
        if (viewGroup instanceof NotificationStackScrollLayout) {
            return ((NotificationStackScrollLayout) viewGroup).getLastChildNotGone();
        }
        if (viewGroup instanceof DesktopNotificationStackScrollLayout) {
            return ((DesktopNotificationStackScrollLayout) viewGroup).getLastChildNotGone();
        }
        Log.w("StackStateAnimator", "getLastChildNotGone with wrong host: " + this.mHostLayout);
        return null;
    }

    private void onChildAnimationFinished() {
        ViewGroup viewGroup = this.mHostLayout;
        if (viewGroup instanceof NotificationStackScrollLayout) {
            ((NotificationStackScrollLayout) viewGroup).onChildAnimationFinished();
        } else if (viewGroup instanceof DesktopNotificationStackScrollLayout) {
            ((DesktopNotificationStackScrollLayout) viewGroup).onChildAnimationFinished();
        } else {
            Log.w("StackStateAnimator", "onChildAnimationFinished with wrong host: " + this.mHostLayout);
        }
    }

    private float getCurrentOverScrollAmount(boolean z) {
        ViewGroup viewGroup = this.mHostLayout;
        if (viewGroup instanceof NotificationStackScrollLayout) {
            return ((NotificationStackScrollLayout) viewGroup).getCurrentOverScrollAmount(z);
        }
        Log.w("StackStateAnimator", "getCurrentOverScrollAmount with wrong host: " + this.mHostLayout);
        return 0.0f;
    }
    ...
    public void setOverScrollAmount(float f, boolean z, boolean z2, boolean z3, boolean z4) {
        ViewGroup viewGroup = this.mHostLayout;
        if (viewGroup instanceof NotificationStackScrollLayout) {
            ((NotificationStackScrollLayout) viewGroup).setOverScrollAmount(f, z, z2, z3, z4);
            return;
        }
        Log.w("StackStateAnimator", "setOverScrollAmount with wrong host: " + this.mHostLayout);
    }

    private boolean isFullySwipedOut(ExpandableView expandableView) {
        ViewGroup viewGroup = this.mHostLayout;
        if (viewGroup instanceof NotificationStackScrollLayout) {
            return ((NotificationStackScrollLayout) viewGroup).isFullySwipedOut(expandableView);
        }
        if (viewGroup instanceof DesktopNotificationStackScrollLayout) {
            return ((DesktopNotificationStackScrollLayout) viewGroup).isFullySwipedOut(expandableView);
        }
        Log.w("StackStateAnimator", "isFullySwipedOut with wrong host: " + this.mHostLayout);
        return false;
    }
}
