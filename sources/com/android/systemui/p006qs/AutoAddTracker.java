...
public class AutoAddTracker implements UserAwareController {
    private static final String[][] CONVERT_PREFS = {new String[]{"QsHotspotAdded", "hotspot"}, new String[]{"QsDataSaverAdded", "saver"}, new String[]{"QsInvertColorsAdded", "inversion"}, new String[]{"QsWorkAdded", "work"}, new String[]{"QsNightDisplayAdded", "night"}, new String[]{"QsAudioFXAdded", "custom(com.motorola.audiofx/com.motorola.motoaudio.tileservice.AudioEffectTileService)"}};
    ...
    public void initialize() {
        if (this.mUserId == 0) {
            for (String[] strArr : CONVERT_PREFS) {
                if (Prefs.getBoolean(this.mContext, strArr[0], false)) {
                    setTileAdded(strArr[1]);
                    Prefs.remove(this.mContext, strArr[0]);
                }
            }
        }
