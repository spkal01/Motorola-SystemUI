...
public class TvPipNotificationController {
    ...
    public TvPipNotificationController(Context context, PipMediaController pipMediaController, Handler handler) {
        ...
        this.mNotificationBuilder = new Notification.Builder(context, "TVPIP").setLocalOnly(true).setOngoing(false).setCategory("sys").setShowWhen(true).setSmallIcon(C2219R.C2221drawable.pip_icon).extend(new Notification.TvExtender().setContentIntent(createPendingIntent(context, "com.android.wm.shell.pip.tv.notification.action.SHOW_PIP_MENU")).setDeleteIntent(createPendingIntent(context, "com.android.wm.shell.pip.tv.notification.action.CLOSE_PIP")));
        pipMediaController.addMetadataListener(new TvPipNotificationController$$ExternalSyntheticLambda0(this));
    ...
    public void onConfigurationChanged(Context context) {
        this.mDefaultTitle = context.getResources().getString(C2219R.string.pip_notification_unknown_title);
        if (this.mNotified) {
            update();
        }
    }
