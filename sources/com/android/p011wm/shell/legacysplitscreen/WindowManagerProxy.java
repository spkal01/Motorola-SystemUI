...
class WindowManagerProxy {
    ...
    public boolean buildEnterSplit(WindowContainerTransaction windowContainerTransaction, LegacySplitScreenTaskListener legacySplitScreenTaskListener, LegacySplitDisplayLayout legacySplitDisplayLayout) {
        ...
        for (int size = rootTasks.size() - 1; size >= 0; size--) {
            ...
            if (runningTaskInfo.supportsMultiWindow || runningTaskInfo.topActivityType == 2) {
                int windowingMode = runningTaskInfo.getWindowingMode();
                if (ArrayUtils.contains(CONTROLLED_WINDOWING_MODES, windowingMode) && ArrayUtils.contains(CONTROLLED_ACTIVITY_TYPES, runningTaskInfo.getActivityType()) && windowingMode != 4) {
                    isHomeOrRecentTask(runningTaskInfo);
                    windowContainerTransaction.reparent(runningTaskInfo.token, legacySplitScreenTaskListener.mSecondary.token, true);
                }
            }
        }
        windowContainerTransaction.reorder(legacySplitScreenTaskListener.mSecondary.token, true);
        return applyHomeTasksMinimized(legacySplitDisplayLayout, (WindowContainerToken) null, windowContainerTransaction);
    }
